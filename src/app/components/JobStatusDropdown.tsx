...
export const JobStatusDropdown = ({
  ...
  const buildButtonClassName = () => {
    let baseClasses =
      "inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium border transition-all duration-200 min-w-[110px] justify-between";
  ...
  const buildOptionClassName = (option: any) => {
    let baseClasses =
      "w-full px-3 py-2.5 text-left text-sm transition-colors flex items-center gap-3 border-none";
            ...
            {isUpdating ? (
              ...
              <div
                className={`w-2 h-2 rounded-full ${
                  selectedOption?.dotColor || "bg-gray-400"
                }`}
              />
            )}
